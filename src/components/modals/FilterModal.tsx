...
interface FilterModalProps {
  ...
  onClear: () => void;
  onApply: () => void;
  fields?: Array<'status' | 'property' | 'startDate' | 'endDate' | 'sortBy'>; 
  dropdownData?: {
    [key: string]: { label: string; value: string }[]  // dynamic data for dropdowns
  }
}

const FilterModal: React.FC<FilterModalProps> = ({ visible, onClose, onClear, onApply,fields,dropdownData }) => {
...
const defaultSortByOptions = [
  { label: 'Property Name (A-Z)', value: 'Asc' },
  { label: 'Property Name (Z-A)', value: 'Desc' },
];
    const statusOptions = [
...
const showProperty = fields?.includes('property');
const showStartDate = fields?.includes('startDate');
const showEndDate = fields?.includes('endDate');
const showSortBy = fields?.includes('sortBy');
  return (
    <Modal
      animationType="slide"
      transparent={true}
...
{showSortBy && (
  <View className="mb-6">
    <Text className="text-sm mb-2" style={{ color: AppColors.black }}>Sort By</Text>
    <Dropdown
      data={dropdownData?.sortBy || defaultSortByOptions}
      labelField="label"
      valueField="value"
